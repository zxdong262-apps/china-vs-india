  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      },
      {
        '@type': 'Dataset',
        'name': locale.site.title,
        'description': `Comparison of key statistics between China and India. ${locale.site.description}`,
        'url': canonicalUrl,
        'inLanguage': localeCode.replace('_', '-'),
        'dateModified': new Date().toISOString().split('T')[0],
        ...
        'dataSource': [
          ...
          {
            '@type': 'Organization',
            'name': 'Ministry of Statistics and Programme Implementation (India)',
            'url': 'https://mospi.nic.in/'
          }
        ],
        'keywords': ['China statistics', 'India comparison', 'population', 'GDP', 'economy', 'energy', '中国统计数据', '印度经济对比'],
      ...
      },
      {
        ...
        'about': {
          ...
          'description': `Comprehensive statistical comparison between China and India covering ${metrics.metrics.length} key metrics including population, economy, energy, agriculture, and manufacturing.`
        }
      }
    ]
  };
